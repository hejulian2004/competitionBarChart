    ...
    function renderFrame(frame, animate = true) {
      ...
      if (isDanmakuEnabled() && danmakuMap.has(frame.time)) {
        ...
        if (hasTitle) {
          const subtitleText = document.querySelector("#subtitleInput").value.trim();
          const maxAvailableCardWidth = Math.min(640, WIDTH - CHART_SIDE_PADDING * 2 - 20);
          const cardWidth = Math.max(280, maxAvailableCardWidth);

          const needsWrapY = WIDTH < 960 && subtitleText;
          const cardX = needsWrapY
            ? TITLE_LEFT
            : Math.max(TITLE_LEFT + 220, WIDTH - CHART_SIDE_PADDING - cardWidth - 10);
          const cardY = needsWrapY ? 86 : 32;
          const cardHeight = 64;
    ...
    function drawDirectCanvasVideoFrame(
      ...
      context.fillText(
        ...
        logicalHeight - margin.bottom - 16
      );
      context.restore();
      ...
      if (isDanmakuEnabled() && danmakuMap.has(frame.time)) {
        ...
        if (hasTitle) {
          ...
          context.fillText(text, cardX + 26, cardY + 47, cardWidth - 36);
          context.restore();
        } else {
          const startX = TITLE_LEFT;
          const startY = 34;
