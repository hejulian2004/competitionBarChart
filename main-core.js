    ...
    function smoothAxisTargetSeries(targets) {
      const minimumMagnitude = getDynamicAxisMinimumMagnitude();
      return targets.map(target => Math.max(minimumMagnitude, target));
    }
    ...
    function updateDynamicAxisTimeline() {
      ...
      const symmetricSeries =
        smoothAxisTargetSeries(symmetricTargets);

      const negativeSeries =
        smoothAxisTargetSeries(negativeTargets);

      const positiveSeries =
        smoothAxisTargetSeries(positiveTargets);
