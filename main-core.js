    ...
    function updateResponsiveChartMargins() {
      ...
      const hasDanmaku = isDanmakuEnabled() && danmakuMap.size > 0;
    ...
    function renderFrame(frame, animate = true) {
      ...
      if (!animate) {
        ...
        const oldTime = timeLabel.node().textContent || frame.time;
    ...
    function renderDanmakuDOM(group, time, text) {
      ...
      const hasTitle = Boolean(document.querySelector("#titleInput").value.trim());
      const isPortrait = WIDTH < 960;
      const startX = (isPortrait || !hasTitle)
        ? TITLE_LEFT
        : Math.max(TITLE_LEFT + 220, WIDTH - CHART_SIDE_PADDING - 520);
      ...
      const textWidth = measureLogicalText(text, 18, 800);
      const maxW = WIDTH - startX - CHART_SIDE_PADDING - 10;
      if (textWidth > maxW) {
    ...
    async function exportVideo() {
      ...
      try {
        ...
        const coverValue = document.querySelector("#videoCoverFrameInput")?.value || "last";
        let coverFrame = null;

        if (coverValue === "last" && raceData.length > 0) {
          coverFrame = raceData[raceData.length - 1];
        } else if (coverValue === "first" && raceData.length > 0) {
          coverFrame = raceData[0];
        } else if (coverValue.startsWith("node:") && raceData.length > 0) {
          const targetTime = coverValue.replace("node:", "");
          coverFrame = raceData.find(f => String(f.time) === targetTime) || raceData[raceData.length - 1];
        }
